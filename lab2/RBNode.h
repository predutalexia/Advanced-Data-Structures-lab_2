...
struct RBTree {
	...
	int maxBlackKey() {
		int result = -1000;
		maxBlackKeyHelper(root, result);
		return result;
	}

	void maxBlackKeyHelper(RBNode* x, int& result) {
		if (isNil(x)) return;
		if (x->col == RBNode::BLACK && x->key > result)
			result = x->key;
		maxBlackKeyHelper(x->left, result);
		maxBlackKeyHelper(x->right, result);
	}
	...
	int maxRedKey() {
		int result = -1000;
		maxRedKeyHelper(root, result);
		return result;
	}

	void maxRedKeyHelper(RBNode* x, int& result) {
		if (isNil(x)) return;
		if (x->col == RBNode::RED && x->key > result)
			result = x->key;
		maxRedKeyHelper(x->left, result);
		maxRedKeyHelper(x->right, result);
	}
};
